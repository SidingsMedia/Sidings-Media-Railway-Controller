...
function saveSettings(){
    ...
    if(ipCheck.test(ipAddress) === false){
        ...
        fs.promises.mkdir(path.join(outDir, 'smrc'), {recursive: true}).then(x => fs.promises.writeFile(path.join(outDir, 'smrc', 'settings.json'), data))
        var el = document.createElement('div')
...
window.onload = function() {
    var outDir = process.env.LOCALAPPDATA || (process.platform == 'darwin' ? process.env.HOME + '/Library/Preferences' : process.env.HOME + "/.local/share")//Get outdir depending on os
    try{
        var rawData = fs.readFileSync(path.join(outDir, 'smrc', 'settings.json'))
